                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <el-button icon="el-icon-upload" size="small"
                                   @click="upload_geodata_layer(scope.row.name, scope.$index)"></el-button>
...
<script lang='ts'>
    ...
    export default Vue.extend({
        ...
        methods: {
            ...
            async upload_geodata_layer(name: any, index: any) {
                const instance = this.$store.state.instance.instance._id;
                console.log('Instance',instance)
                const level = name;
                const geojson = geodata_cache[name];
                if (!instance) {
                ...
                this.$notify.success({
                    message: `Saving ${name} Geodata `
                });
                try{
                    await this.$store.dispatch('geodata/create',{instance,level,geojson});
                    this.$notify.success({
                        message: `Succefully saved ${name} Geodata `
                    });
                }catch (e) {
                    this.$notify.error({
                        message: `Failed to upload ${name} Geodata `
                    });
                }
            },
